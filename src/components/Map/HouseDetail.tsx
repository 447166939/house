...
<IconButton css={styles.heartBtn}><Image css={styles.heartIcon} src={heartIcon} alt={''}/></IconButton>
        <IconButton><Image css={styles.shareIcon} src={shareIcon} alt={''}/></IconButton>
        <IconButton><Image css={styles.eyeIcon} src={eyeIcon} alt={''}/></IconButton>
    </Box>
    ...
    <Box css={styles.saleBox}>
        ...
        <Box css={styles.saleText}><span css={styles.saleName}>Zestimatee：</span><span css={styles.saleMoney}>$143,300</span></Box>
    </Box>
    ...
    <Box css={styles.buttonBox}>
        <Button css={styles.createBtn} variant={'contained'}>创建REI项目</Button>
        <GreenButton css={styles.backupBtn} variant={'contained'}>存入备选</GreenButton>
        <GreyButton css={styles.documentBtn} variant={'contained'}>存入档案</GreyButton>
    </Box>
    <Box css={styles.overviewTab}>
        {detail.links.map((item:any,index:number)=>(
            <Box onClick={handleChangeOverviewTab.bind(null,index)} css={styles.overviewTabItem({isActive:idx==index})} key={index}>
                {item.text}
            </Box>
        ))}
    </Box>
    ...
    <Box css={styles.viewerBox}>
        <Box><span css={styles.daynum}>{detail.day}</span><span css={styles.dayText}> day on Zillow</span></Box>
        <Box><span css={styles.viewnum}>{detail.views}</span><span css={styles.viewText}> views</span></Box>
        <Box><span css={styles.savenum}>{detail.save}</span><span css={styles.saveText}> saves</span></Box>
    </Box>
    </Box>
</Box>
